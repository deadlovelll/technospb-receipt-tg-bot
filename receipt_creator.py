...
class ReceiptCreator:
    ...
    def create_pdf(filename, items):
        ...
        # Draw QR code image
        qr_code_path = 'QRtiger.png'  # Replace with your QR code image path
        ...
        stamp_path = '/content/171880989613650406.jpeg'
        stamp_width = 2.5 * inch
        stamp_height = 2.0 * inch

        # Save the current state of the canvas
        c.saveState()

        # Rotate the canvas by -15 degrees (counter-clockwise)
        c.rotate(5)

        # Draw the image
        c.drawImage(stamp_path, 0.5 * inch, y_start - 2.75 * inch, width=stamp_width, height=stamp_height)
        ...
        c.setFont('PTMono', 10)
        c.drawString(0.2 * inch, y_start - 2.35 * inch, "М.П.")

        c.line(0.5 * inch, y_start - 2.4 * inch, 2.5 * inch, y_start - 2.4 * inch)

        # Seller's signature
        c.drawString(3.1 * inch, y_start - 2.1 * inch, "Иванков Т.")
        c.drawString(3.1 * inch, y_start - 2.4 * inch, "Расшифровка")
